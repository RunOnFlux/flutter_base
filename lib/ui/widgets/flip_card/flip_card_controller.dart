...
class FlipCardController {
  ...
  Future<void> flip({CardSide? targetSide}) async => await state?.flip(targetSide);
  ...
  Future<void> skew(double target, {Duration? duration, Curve? curve}) async =>
      await state?.skew(target, duration: duration, curve: curve);
  ...
  }) async => await state?.hint(target: target, duration: duration, curveTo: curveTo, curveBack: curveBack);
}
