...
class FlipCardState extends State<FlipCard> with SingleTickerProviderStateMixin {
  ...
  /// Return the current side being shown
  CardSide getSide() {
    return CardSide.fromAnimationStatus(controller.status);
  }
  ...
  Future<void> skew(double target, {Duration? duration, Curve? curve}) async {
    ...
    if (target > controller.value) {
      await controller.animateTo(target, duration: duration, curve: curve ?? Curves.linear).complete;
    } else {
      await controller.animateBack(target, duration: duration, curve: curve ?? Curves.linear).complete;
    }
  }
  ...
  Future<void> hint({
    ...
    final halfDuration = Duration(milliseconds: (duration.inMilliseconds / 2).round());

    try {
      await controller.animateTo(target, duration: halfDuration, curve: curveTo).complete;
    } finally {
      await controller.animateBack(0, duration: halfDuration, curve: curveBack).complete;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.flipOnTouch) {
      return GestureDetector(behavior: HitTestBehavior.translucent, onTap: flip, child: child);
    }
